...
export const FLAG_GROUPS: FlagGroup[] = [
  ...
  },
  {
    ...
    // 이름에 횟수를 넣지 않는다 — 기준은 피벗(저점·고점) 개수인데 "3회"로 적으면
    // 다이버전스가 3번 났다는 뜻으로 읽힌다 (저점 3개 = 다이버전스 2쌍).
    name: "연속 다이버전스",
    flags: [
      { key: "div_reg_bull_x3", label: "상승 다이버전스 연속", short: "상승다이버 연속", bullish: true, desc: "가격 저점이 계속 낮아지는데 RSI 저점은 계속 높아지는 상태가 3개 이상의 저점에 걸쳐 이어짐. 하락 동력이 반복적으로 약해지고 있다는 신호로, 한 번 나온 것보다 강하게 봅니다." },
      { key: "div_reg_bear_x3", label: "하락 다이버전스 연속", short: "하락다이버 연속", bullish: false, desc: "가격 고점이 계속 높아지는데 RSI 고점은 계속 낮아지는 상태가 3개 이상의 고점에 걸쳐 이어짐. 상승 동력이 반복적으로 꺼지고 있다는 신호로, 한 번 나온 것보다 강하게 봅니다." },
      { key: "div_hid_bull_x3", label: "히든 상승 연속", short: "히든상승 연속", bullish: true, desc: "상승 추세 눌림목에서 히든 상승 다이버전스가 3개 이상의 저점에 걸쳐 반복됨. 상승 추세가 계속 유지되고 있다는 신호." },
      { key: "div_hid_bear_x3", label: "히든 하락 연속", short: "히든하락 연속", bullish: false, desc: "하락 추세 반등에서 히든 하락 다이버전스가 3개 이상의 고점에 걸쳐 반복됨. 하락 추세가 계속 유지되고 있다는 신호." },
    ],
  },
  {
    name: "MACD",
  ...
  {
    // 그물차트 — 국내 관행(5·10·…·100일 SMA 20개). 종가가 스무 선 위에
    // 전부 있으면 정배열, 전부 아래면 역배열. 다른 시그널과 달리 '오늘 터진
    // 사건'이 아니라 며칠씩 이어지는 상태다 (실측 정배열 17.8%, 역배열 25.9%).
    name: "그물차트",
    flags: [
      { key: "gmma_above", label: "그물망 위 (정배열)", short: "그물망 위", bullish: true, desc: "종가가 5·10·…·100일 이동평균선 스무 개 위에 모두 있는 상태입니다. 짧게 보든 길게 보든 평균보다 비싸다는 뜻이라 상승 추세로 봅니다. 한 번 나오면 며칠씩 이어지는 상태이고, 전체 종목의 5분의 1 정도가 여기 해당합니다." },
      { key: "gmma_below", label: "그물망 아래 (역배열)", short: "그물망 아래", bullish: false, desc: "종가가 5·10·…·100일 이동평균선 스무 개 아래에 모두 있는 상태입니다. 짧게 보든 길게 보든 평균보다 싸다는 뜻이라 하락 추세로 봅니다. 한 번 나오면 며칠씩 이어지는 상태이고, 전체 종목의 4분의 1 정도가 여기 해당합니다." },
    ],
  },
  {
    ...
    name: "이격도",
    flags: [
      { key: "disp20_low", label: "20일 이격도 침체 (≤85)", short: "이격20↓", bullish: true, desc: "종가가 20일 이동평균보다 15% 이상 아래로 벌어진 상태. 단기 낙폭이 과해 평균선 쪽으로 되돌릴 여지가 커지는 구간으로 봅니다." },
      { key: "disp20_high", label: "20일 이격도 과열 (≥115)", short: "이격20↑", bullish: false, desc: "종가가 20일 이동평균보다 15% 이상 위로 벌어진 상태. 단기 상승이 과해 평균선 쪽으로 되돌릴 여지가 커지는 구간으로 봅니다." },
      { key: "disp60_low", label: "60일 이격도 침체 (≤75)", short: "이격60↓", bullish: true, desc: "종가가 60일 이동평균보다 25% 이상 아래로 벌어진 상태. 중기 추세에서 크게 이탈했다는 뜻입니다. 20일보다 기준이 훨씬 넓은 건 추세가 붙은 종목이 60일선에서 오래 떨어져 있기 때문입니다." },
      { key: "disp60_high", label: "60일 이격도 과열 (≥130)", short: "이격60↑", bullish: false, desc: "종가가 60일 이동평균보다 30% 이상 위로 벌어진 상태. 중기 추세에서 크게 앞서 나갔다는 뜻입니다. 20일보다 기준이 훨씬 넓은 건 추세가 붙은 종목이 60일선에서 오래 떨어져 있기 때문입니다." },
    ],
  },
  {
    name: "상승 패턴 (완성)",
  ...
  },
  {
    ...
    flags: [
      ...
      { key: "cdl_harami_bull", label: "상승 잉태형", short: "상승잉태", bullish: true, desc: "큰 음봉 몸통 안에 작은 양봉이 들어간 모양. 장악형과 반대 구조로, 하락 힘이 잦아들고 있다는 신호로 봅니다." },
      { key: "cdl_inv_hammer", label: "역망치형 (하락 후)", short: "역망치", bullish: true, desc: "하락 뒤 위꼬리가 긴 캔들. 유성형과 모양은 같지만 바닥에서 나오면 반등 시도로 해석합니다." },
      { key: "cdl_tweezer_bottom", label: "족집게 바닥", short: "족집게바닥", bullish: true, desc: "이틀 저가가 사실상 같은 자리에서 멈춘 모양. 그 가격대가 지지로 확인됐다는 신호." },
      { key: "cdl_3soldiers", label: "적삼병 (3봉)", short: "적삼병", bullish: true, desc: "몸통이 큰 양봉 세 개가 계단처럼 이어진 모양. 매수세가 사흘 연속 이어졌다는 뜻입니다." },
      { key: "cdl_harami_bear", label: "하락 잉태형", short: "하락잉태", bullish: false, desc: "큰 양봉 몸통 안에 작은 음봉이 들어간 모양. 상승 힘이 잦아들고 있다는 신호로 봅니다." },
      { key: "cdl_hanging", label: "교수형 (상승 후)", short: "교수형", bullish: false, desc: "상승 뒤 아래꼬리가 긴 캔들. 망치형과 모양은 같지만 고점에서 나오면 경고로 해석합니다." },
      { key: "cdl_tweezer_top", label: "족집게 천장", short: "족집게천장", bullish: false, desc: "이틀 고가가 사실상 같은 자리에서 막힌 모양. 그 가격대가 저항으로 확인됐다는 신호." },
      { key: "cdl_3crows", label: "흑삼병 (3봉)", short: "흑삼병", bullish: false, desc: "몸통이 큰 음봉 세 개가 계단처럼 이어진 모양. 매도세가 사흘 연속 이어졌다는 뜻입니다." },
      { key: "cdl_wick_top", label: "긴 윗꼬리 반복", short: "윗꼬리반복", bullish: false, desc: "고점권에서 위꼬리가 긴 봉이 10봉 안에 3번 이상 나온 상태. 오를 때마다 위에서 눌리는 일이 되풀이됐다는 뜻입니다. 거래량이 20일 평균 이상인 봉만 셉니다 — 거래 없이 생긴 위꼬리는 매도 압력이 아니라 한산해서 생긴 자국이기 때문입니다." },
