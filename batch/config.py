...
CHART_PRO_DIR = OUTPUT_DIR / "chart2y"   # 구독자용 2년치
...
# 그물차트 — 국내 관행(네이버·HTS)인 (시작 5, 간격 5, 개수 20)을 쓴다.
# 단순이동평균 5·10·…·100일 스무 개.
#
# 2026-08-25 변경. 처음에는 Guppy 원안(EMA 3~60, 12개)으로 만들었는데,
# 같은 종목을 네이버에서 보면 그물이 훨씬 넓게 퍼져 모양이 아예 달랐다
# (삼성전자 마지막 봉 퍼짐 14,945원 vs 42,633원). 이름이 같은데 그림이
# 다르면 이 앱이 틀린 것처럼 읽힌다.
#
# 종가가 스무 선 위에 다 있으면 정배열, 다 아래면 역배열.
# 이벤트가 아니라 상태다 — 실측 정배열 17.8%, 역배열 25.9% (349종목 13.7만 봉).
GMMA_START, GMMA_STEP, GMMA_COUNT = 5, 5, 20
GMMA_PERIODS = tuple(GMMA_START + GMMA_STEP * i for i in range(GMMA_COUNT))
GMMA_SHORT = GMMA_PERIODS[: GMMA_COUNT // 2]   # 5~50, 화면에서 한 색
GMMA_LONG = GMMA_PERIODS[GMMA_COUNT // 2 :]    # 55~100
GMMA_MIN_BARS = GMMA_PERIODS[-1]  # 가장 긴 선이 자리를 잡기 전에는 판정하지 않는다
...
# 2026-08-25: 앱이 일봉만 쓰기로 하면서 주봉·월봉 산출을 멈췄다.
# 종목당 107KB 중 주봉이 47%·월봉이 11%였다 — 아무도 안 보는 데이터를
# 매일 만들어 매번 내려보내고 있었다.
#
# 무료는 1년, 구독은 2년. 앱이 종목을 넘길 때마다 받는 파일이라
# 봉 수가 곧 전송량이다 (1년 30KB · 2년 46KB, 종전 107KB).
CHART_DAILY_BARS = 250      # 무료 — 1년
CHART_DAILY_BARS_PRO = 500  # 구독 — 2년
