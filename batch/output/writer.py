...
def write_chart(
    ...
    """종목 상세 차트 json. tf = {"d": payload} — 일봉만.

    무료는 chart/{code}.json(1년), 구독은 chart2y/{code}.json(2년).
    한 파일에 2년을 담고 앱이 잘라 쓰게 하면, 무료 사용자도 2년치를 받게 돼
    아끼는 뜻이 없어진다.
    ...
    out = config.CHART_PRO_DIR if pro else config.CHART_DIR
