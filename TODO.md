...
- [ ] **[보류]** 10년 백필: 수집 소스가 장기간 요청에서 간헐 행업 (소켓 타임아웃 20s 방어는
  반영됨, 279/2578종목은 10년치 확보). 재개 방법: `BACKFILL_YEARS=10` + 워크플로 캐시 키에
  버전 접두어 추가 + 야간·저부하 시간대에 분할 재구축. 당분간 주봉은 2년 범위로 서비스
- [ ] CI 배포 검증 (오전 크론이 v2 코드로 산출하는지)

### C3. 백테스팅 v1 — 시그널 성과 통계

- [ ] 과거 시그널 이벤트 스캔 (전 종목 × 10년)
- [ ] 사후 수익률(5/10/20/60일) 집계 → stats/signals.json
- [ ] /stats 페이지 (성과 표 + "과거 통계, 수익 보장 아님" 면책)
- [ ] 종목 상세·프리셋 페이지에 통계 섹션
- [ ] 표본 종목 수기 대조 검증
