...
- [ ] 👤 공공데이터포털 가입 + 금융위원회_주식시세정보 API 활용신청, 키 발급 → `.env`에 `DATA_GO_KR_API_KEY` 기입
- [x] `.env.example` 작성
...
- [x] 공공 API 클라이언트 (`batch/collector/daily.py`) — **키 발급 후 실호출 검증 필요**
- [x] 일별 수집: 공공 API 우선 + fdr 증분 폴백, 휴장일 처리
- [x] 검증: fdr 수집값 정상 확인 (공공 API와 대조는 키 발급 후)
