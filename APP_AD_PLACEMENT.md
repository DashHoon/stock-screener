...
| `lib/main.dart` | 온보딩, 데이터 없는 경우 Splash, 이후 Shell | 실제 데이터 로딩 또는 시작 후 4초의 첫 시장 화면에서만 오프닝 광고를 허용하고 본문은 지연하지 않음 |
...
| 검색 결과 | 적응형 배너 1개 | 검색 본문 아래, 하단 탭 위; 정상 데이터가 있고 키보드가 닫힌 경우 | 조건·범위·결과 본문을 대체하거나 덮는 영역, 검색 입력 중 |
| 검색 조건 | 적응형 배너 1개 | 조건 목록 아래의 별도 하단 영역; 뒤쪽 검색 배너는 비활성화 | 탭·시그널 목록·적용 버튼을 덮는 영역 |
| 업종 상세 | 네이티브 | 종목 6·18·30개 뒤; 차트·설명은 개수에서 제외 | 업종 차트나 지표 토글 사이 |
| 관심종목 | 적응형 배너 1개 | 목록 본문 아래, 하단 탭 위 별도 영역; 정상 데이터 및 비편집 상태 | 추가·삭제·이름 변경·편집·키보드 입력 중 |
...
검색·검색 조건·관심종목 화면에서 large anchored adaptive 배너를 사용한다. 광고 플랫폼 뷰는 SDK가 반환한 실제 광고 높이와 위·아래 16dp 여백을 합친 `ClipRect` 전용 영역 안에 고정해 본문을 덮을 수 없게 한다. 검색에서는 조건·결과 본문을 항상 먼저 렌더링하고 배너는 하단 탭 위의 별도 영역에만 둔다. 검색 조건 화면을 열면 뒤쪽 검색 배너를 끄고 조건 화면의 하단 배너 하나만 활성화한다. 이름 검색으로 키보드가 열린 동안에는 숨긴다. 작은 화면·큰 글자·제스처 내비게이션에서 탭 오클릭 가능성이 있으면 배너를 생략한다. [Google 배너 배치 안내](https://support.google.com/admob/answer/6275345?hl=en)
...
- 콜드 스타트는 두 번째 실행부터 실제 데이터 로딩 중이거나 시작 후 4초 안에 첫 시장 화면에 머물러 있을 때 광고가 준비되면 표시한다. 광고 준비를 위해 앱 본문을 인위적으로 기다리게 하지 않는다.
...
- 사용자가 다른 탭으로 이동하면 시작 광고 기회를 즉시 취소한다. 4초가 지나거나 첫 화면을 벗어난 뒤 늦게 로드된 광고는 폐기한다.
...
| `app_open_first_launch_excluded` / `app_open_opportunity_sec` | 1 / 4 | 첫 실행 제외; 본문 지연 없는 첫 화면 광고 기회 |
...
- 앱 오프닝은 시작 후 4초 또는 첫 탭 이동 뒤에는 표시되지 않고 외부 뉴스·메일·광고에서 복귀하면 생략된다.
...
| 두 번째 콜드 스타트, 실제 데이터 로딩 중 오프닝 준비 | 오프닝 노출과 동시에 데이터 로딩 → 닫으면 준비된 본문 표시 → Release에서는 닫힘 후 180초 미만이면 전면 생략 |
